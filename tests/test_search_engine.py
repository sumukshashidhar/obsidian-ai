...
class TestExactPhraseSearch:
  def test_exact_phrase_found(self, mock_config):
    search = ExactPhraseSearch()
    results = search.search("Tara comes to the US", max_results=10)

    assert len(results) >= 1
    assert any("2025-08-18.md" in result.path for result in results)
    assert any("Tara comes to the US" in result.text for result in results)
