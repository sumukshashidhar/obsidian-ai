...
class ResearchAgent:
  ...
  def _execute_research(self, topic: str, max_iterations: int) -> dict[str, Any]:
    ...
    return {
      ...
      "discovered_entities": sorted(list(discovered_entities)),
      "research_steps": [
        {
  ...
  def _plan_research_step(self, topic: str, current_focus: str, previous_steps: list[ResearchStep], discovered_entities: set[str]) -> dict[str, str]:
    ...
    if discovered_entities:
      discovered_context = f"\n\n**Entities Discovered So Far:** {', '.join(sorted(list(discovered_entities))[:10])}"
  ...
  def _execute_search(self, action_plan: dict[str, str]) -> tuple[list[dict], list[str]]:
    ...
    try:
      if action_plan["action"] == "search":
        ...
        search_results = [{"path": result.relative_path, "line": result.line, "text": result.text, "score": result.score} for result in results]
